 ...
 *
 * It lives in the Shared Kernel because the bucket is not the container's
 * private resource: the edge reads the same objects, and a second client would
 * be a second set of rules about what a conditional write means (docs/adr/0010).
 * `FileStore` is the one implementation that stays in `src/store.ts` — it is
 * the local disk, which only the container has.
 *
