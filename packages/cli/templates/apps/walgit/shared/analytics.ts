 ...
 *
 * What the snippet is told: no session recording and no person profiles for
 * anonymous readers. The page has no consent banner, and a git host for agents
 * that records strangers' screens would be making a claim about privacy that
 * the rest of this deployment does not make.
 */

/** The variables analytics is read from, and only those. */
export type AnalyticsVar = 'WALGIT_POSTHOG_KEY' | 'WALGIT_POSTHOG_HOST'
...
export type Analytics = {
  ...
  key: string
  /** The ingestion origin. PostHog Cloud US unless the instance says EU or self-hosted. */
  host: string
}
...
export function analyticsFrom(
  ...
  return { key, host: trimmed(env.WALGIT_POSTHOG_HOST) ?? DEFAULT_POSTHOG_HOST }
}
...
export function analyticsScript(analytics: Analytics | null): string {
  ...
  return (
    ...
    `posthog.init(${key},{api_host:${host},person_profiles:"identified_only",disable_session_recording:true});\n` +
    '</script>\n'
  )
}
