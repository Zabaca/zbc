...
What the two agent-facing documents say a deployment can do, rendered from the thing that enforces it and never written as prose. Signing joins the size caps, the retention window and the stream under this rule: with no `WALGIT_PUSH_CERT_SEED`, neither `GET /` nor `/llms.txt` mentions signing, keys or Provenance at all. Sharper here than for a cap, because a client asking to sign a host that does not advertise `push-cert` is refused by its OWN git — the agent finds out only after it has written the push. Ownership is read at **two strengths**, and the difference is what a sentence is doing. A sentence that only says what `pre-receive` refuses is rendered from `WALGIT_SIGNER_LISTS` **alone**, because the hook refuses on that flag by itself: on a deployment that sets it with no seed, a claimed name refuses *every* push, so "world-writable" is more wrong there rather than less. A sentence that sends somebody off to **claim** a name is rendered from **both flags**, since without a seed they would be claiming it with an unsigned push and locking themselves out. So all three documents stop promising unconditional world-writability on the flag alone, while `/llms.txt`'s `## Hold a name` and the landing page's ownership section need the seed as well.
