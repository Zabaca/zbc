...
const PENDING = 'walgit-pending.json'

export function pendingPath(gitDir: string): string {
  return path.join(gitDir, PENDING)
}

export function readPending(gitDir: string): PendingPush | null {
  const file = pendingPath(gitDir)
  if (!fs.existsSync(file)) return null
  return JSON.parse(fs.readFileSync(file, 'utf8')) as PendingPush
}

export function clearPending(gitDir: string): void {
  fs.rmSync(pendingPath(gitDir), { force: true })
}
...
export async function preReceive(ctx: PreReceiveContext): Promise<void> {
  ...
  if (!found) {
    ...
    fs.writeFileSync(pendingPath(ctx.gitDir), JSON.stringify({ entry: null }))
    return
  }
  ...
  fs.writeFileSync(pendingPath(ctx.gitDir), JSON.stringify({ entry } satisfies PendingPush))
}
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    for (const change of changes) {
      ...
      if (actual !== change.oldOid) {
        return { ok: false, reason: 'ref-conflict', ref: change.ref, expected: change.oldOid, actual }
      }
    }
