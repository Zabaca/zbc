 ...
 * the ref-event stream of docs/adr/0009, and scenario 9 the push provenance of
 * docs/adr/0011.
 */
...
/**
 * The one place the provenance chain is checked without a double anywhere in
 * it (docs/adr/0011).
 *
 * Every seam below is stubbed somewhere in the unit suite, and each stub is
 * reasonable on its own — but the design was nearly built on git's own
 * verification verdict, which reports `GIT_PUSH_CERT_STATUS=N` and no signer at
 * all for an SSH signature. A fake verifier would have reported success against
 * that mistake for as long as anyone cared to look. So here the key is real and
 * generated by this scenario, the certificate is minted by `git push
 * --signed=yes` against a server that advertised the capability, the signature
 * is verified by the real `ssh-keygen`, and the answer is read back over HTTP
 * from the endpoint a client would use — then compared against `ssh-keygen -lf`
 * on the public half of the key, which is the only fingerprint in the run that
 * walgit had no hand in producing.
 */

/** The seed this scenario's node advertises nonces from. Local, and disposable. */
const CERT_NONCE_SEED = 'walgit-e2e-push-cert-seed'

/** `ssh-keygen`, run for its output. Never throws; the caller asserts. */
async function sshKeygen(...args: string[]): Promise<{ status: number; out: string }> {
  const child = Bun.spawn(['ssh-keygen', ...args], { stdout: 'pipe', stderr: 'pipe' })
...
/**
 * How many objects the entries after `seqBefore` carry, straight out of the
 * pack header (`PACK`, version, object count — bytes 8..12, big-endian).
 *
 * Zero when nothing was transferred, which is the case a ref-only push has to
 * be checked against. Reading the header beats counting WAL entries because a
 * signed ref-only push uploads an empty pack where an unsigned one uploads
 * nothing, and both mean the same thing.
 */
async function packObjectCount(run: Run, index: WalIndex, seqBefore: number): Promise<number> {
  let objects = 0
  for (const entry of index.entries.filter((e) => e.seq > seqBefore)) {
    const stored = await run.store.get(entry.key)
    assert(stored !== null, `the log names ${entry.key}, which the store does not have`)
    const head = stored.body
    assert(new TextDecoder().decode(head.subarray(0, 4)) === 'PACK', `${entry.key} is not a pack`)
    objects += new DataView(head.buffer, head.byteOffset, head.byteLength).getUint32(8)
  }
  return objects
}

/** `{ 'refs/heads/main': { signer, ts } }`, read over HTTP as a client would. */
async function readProvenance(
  node: WalgitNode,
  repoId: string,
): Promise<Record<string, Provenance>> {
  const res = await fetch(
    `http://127.0.0.1:${node.port}${PROVENANCE_PATH}?repo=${encodeURIComponent(repoId)}`,
    { headers: { authorization: `Bearer ${TOKEN}` } },
  )
  assert(res.status === 200, `provenance read answered ${res.status}, expected 200`)
  const body = (await res.json()) as { repo: string; provenance: Record<string, Provenance> }
  assert(body.repo === repoId, `provenance read named ${body.repo}, expected ${repoId}`)
  return body.provenance
}

const pushProvenance: Scenario = {
  n: 9,
  name: 'Push provenance — a real signed push names its key, and an unsigned one names nobody',
  async run(run) {
    const repoId = run.repoId('provenance')
    // The seed IS the capability: with it set, `git-receive-pack` advertises
    // `push-cert`; without it the client's own git refuses `--signed=yes`
    // before a byte reaches the network. So a signed push that exits 0 below
    // is itself evidence the server offered the capability.
    const node = await run.node('provenance', { WALGIT_PUSH_CERT_SEED: CERT_NONCE_SEED })

    // Generated here, used here, and gone at the end — walgit has no key
    // registry (that is the point of `check-novalidate`), so nothing outside
    // this scenario ever needs to have heard of it.
    const keyDir = run.dir('signing-key')
    const keyPath = path.join(keyDir, 'id_ed25519')
    try {
      const generated = await sshKeygen(
        '-q',
        '-t',
        'ed25519',
        '-N',
      ...
      assert(generated.status === 0, `ssh-keygen could not generate a key:\n${generated.out}`)

      // The expected answer, from the tool rather than from walgit. `-lf` on
      // the PUBLIC half: if the two disagree, the recorded Signer is not this
      // key, whatever else verified.
      const listed = await sshKeygen('-lf', `${keyPath}.pub`)
      assert(listed.status === 0, `ssh-keygen -lf failed:\n${listed.out}`)
      const expected = fingerprintIn(listed.out)
      assert(expected !== null, `ssh-keygen -lf named no fingerprint:\n${listed.out}`)

      const work = await clone(run, node, repoId, 'signer')
      // Local config only — the ambient `~/.gitconfig` is already stripped by
      // the harness, so this is the whole of what makes the client sign.
      await gitOk(work, 'config', 'gpg.format', 'ssh')
      await gitOk(work, 'config', 'user.signingkey', keyPath)

      // 1. A real signed push, minted and verified end to end.
      const signed = await commit(work, 'signed\n')
      const pushSigned = await git(work, 'push', '--signed=yes', 'origin', 'HEAD:refs/heads/main')
      assert(pushSigned.status === 0, `the signed push failed:\n${pushSigned.out}`)

      const afterSigned = await readProvenance(node, repoId)
      const main = afterSigned['refs/heads/main']
      assert(main !== undefined, 'the signed push recorded no Signer for refs/heads/main')
      assert(
        main.signer === expected,
        `recorded ${main.signer}, but ssh-keygen -lf says ${expected}`,
      )
      assert(
        !Number.isNaN(Date.parse(main.ts)),
        `the recorded timestamp ${main.ts} is not an instant`,
      )

      // 2. A signed push whose refs move without new objects: a second branch
      // pointed at the commit the server already has. The provenance path must
      // not be hung off the upload — everything above would pass if it were.
      const seqBefore = (await loadIndex(run.store, repoId)).index.seq
      const pushRefOnly = await git(
        work,
        'push',
        '--signed=yes',
        'origin',
      ...
      assert(pushRefOnly.status === 0, `the ref-only signed push failed:\n${pushRefOnly.out}`)

      const afterRefOnly = await loadIndex(run.store, repoId)
      assert(
        afterRefOnly.index.refs['refs/heads/release'] === signed,
        'the ref-only push did not move refs/heads/release',
      )
      // "No new objects" is asserted about the objects, not about whether a
      // pack arrived: a signed push sends an EMPTY pack where an unsigned one
      // sends none at all (scenario 8's ref-only push appends no WAL entry;
      // this one appends a 32-byte header-and-trailer). Both are the same
      // fact — nothing was transferred — so the check reads the pack's own
      // object count rather than counting WAL entries.
      const objectsPushed = await packObjectCount(run, afterRefOnly.index, seqBefore)
      assert(
        objectsPushed === 0,
        `the ref-only push transferred ${objectsPushed} object(s); it was not ref-only`,
      )

      const release = (await readProvenance(node, repoId))['refs/heads/release']
      assert(release !== undefined, 'the ref-only signed push recorded no Signer')
      assert(
        release.signer === expected,
        `the ref-only push recorded ${release.signer}, expected ${expected}`,
      )

      // 3. The negative, on the same repository and the same clone: an
      // ordinary push lands, and records nothing — neither a Signer of its
      // own, nor any change to the two already recorded.
      const unsigned = await commit(work, 'unsigned\n')
      const pushUnsigned = await git(work, 'push', 'origin', 'HEAD:refs/heads/plain')
      assert(pushUnsigned.status === 0, `the unsigned push failed:\n${pushUnsigned.out}`)

      const final = await readProvenance(node, repoId)
      const { index } = await loadIndex(run.store, repoId)
      assert(
        index.refs['refs/heads/plain'] === unsigned,
        'the unsigned push did not land — the negative below would be vacuous',
      )
      assert(
        final['refs/heads/plain'] === undefined,
        `the unsigned push recorded ${final['refs/heads/plain']?.signer}`,
      )
      assert(
        Object.keys(final).sort().join(',') === 'refs/heads/main,refs/heads/release',
        `provenance names ${Object.keys(final).sort().join(', ')} after the unsigned push`,
      )
      assert(
        final['refs/heads/main']?.ts === main.ts,
        'the unsigned push disturbed the Signer of a ref it never touched',
      )

      // Gone the moment it stops being needed, and observed to be gone rather
      // than assumed: a suite that leaves a private key in a scratch directory
      // has taught every reader that this is how one signs a push.
      fs.rmSync(keyDir, { recursive: true, force: true })
      assert(!fs.existsSync(keyPath), 'the signing key outlived the scenario')

      return [
        `signed push ${signed.slice(0, 8)} recorded ${expected}, which is ` +
          'ssh-keygen -lf on the public half of a key generated by this scenario',
        `the ref-only signed push recorded ${release.signer.slice(0, 14)}… for ` +
          'refs/heads/release, transferring 0 objects',
        `the unsigned push landed as ${unsigned.slice(0, 8)} on refs/heads/plain and ` +
          `recorded nothing; provenance still names exactly ${Object.keys(final).length} refs`,
        'read back over HTTP from ' +
          `${PROVENANCE_PATH}?repo=…, behind the same credential a clone needs`,
        'the ed25519 key was generated by this scenario and removed with it',
      ]
    } finally {
      // The same removal again, for the paths that never reached it: an
      // assertion above throwing must not be how a key survives.
      fs.rmSync(keyDir, { recursive: true, force: true })
    }
  },
}
