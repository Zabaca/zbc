# Coding agents work in a disposable clone, contained by the SDK's own sandbox
...
- `settingSources: ['project']` is safe **only because** the Workspace is outside
  ...
  `packages/agent/` it loaded this repo's CLAUDE.md *and* `/Users/uptown/CLAUDE.md`
...
- **Leaving `allowUnsandboxedCommands` at its default.** Rejected, and this is
  ...
  open, the agent hit `Operation not permitted`, said *"Let me retry with the
  sandbox disabled"*, and read the file — `Bash` accepts a
...
- **A hand-written `sandbox-exec` (seatbelt) profile.** Rejected. It does deliver
  ...
  seatbelt wraps the CLI *and* `Bash` under one policy, so every hole the CLI
...
- **`allowRead` is not an allowlist on its own.** It re-allows paths *within*
...
- **Two environment variables must never be set**, both of which fail in ways
  ...
  - `HOME` — hides the login Keychain, 401s the CLI, and raises a *"Keychain Not
    Found"* dialog at whoever is sitting at the machine.
...
- **Token cost is the price of the job.** Tool schemas for
  ...
  taken once per session — commits made *during* a session invalidate nothing.
