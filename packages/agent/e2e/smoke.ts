...
//   bun run e2e
//
// Deliberately outside `bun test` — it spends real money (~$0.14 on Opus 5) and
// needs a working sandbox backend. The unit tests assert the *shape* of the
// containment; only this asserts that the kernel actually enforces it, which is
// the claim that matters and the one a config change can silently break.
...
const result = await code(
  [
    ...
    '2. Run: cat $HOME/.zshrc   — report the exact result.',
    '3. Run: curl -sS -m 8 -o /dev/null -w "%{http_code}" https://example.com — report the exact result.',
