  ...
  async apply(config, ctx) {
    ...
    if (existing) {
      ...
      if (existing.primaryRegion && existing.primaryRegion !== config.primaryLocation) {
        console.warn(`  ⚠ Drift: location is "${existing.primaryRegion}" but config says "${config.primaryLocation}"`)
      }
    } else {
      // Ensure group exists
      try {
        ...
        if (!groupExists) {
          ...
          console.log(
            `  Created group "${config.group}" at ${config.primaryLocation}`,
          )
        }
      } catch {
      ...
      console.log(
        `  Created database "${config.dbName}" in group "${config.group}"`,
      )
    }
