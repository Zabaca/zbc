...
**MCP Server**:
The fourth thing the Client does: `agentgit mcp` is the same four verbs over stdio JSON-RPC, so a harness calls them instead of shelling out. It is a second front door, never a second implementation — every tool is the function the matching CLI verb already calls (`runAccept`, `runSetup`, `watchOnce`), so a refusal an agent reads is the refusal a person reads, word for word. Two rules are its own: **stdout belongs to the transport**, so nothing in that path prints; and `agentgit_watch_once` carries a deadline where `--once` blocks forever, because a tool call that never returns is a hung session rather than a deliberate wait. It answers reaching the deadline, never throws it.
_Avoid_: "the API", "the server" — there is no service here and nothing is hosted; it is a subprocess a client spawns.
