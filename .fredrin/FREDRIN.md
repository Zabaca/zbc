...
- **goal / milestone / objective → a Fredrin Goal.** When the user wants related
  ...
  - **Take care of dependencies — infer them, don't wait to be told.** Work out
    ...
    `"dependsOn":["<prereq-id>",…]` on each dependent so it lands *blocked by* its
    prerequisites. The dependency tickets get **created too**, not only the
    top-level ones. (`dependsOn` must hold only real, non-empty ticket ids — omit
    it entirely when a ticket has no prerequisites; never pass `[""]`.)
  - **Default a batch-created goal to ship-together.** When you create a goal
    *together with* a set of tickets, enable its staging branch —
    ...
    to `main` once every ticket is complete. Best-effort: skip it if the project
...
numbered, short — see existing entries 0001–0004), and the domain glossary is the
root `CONTEXT.md`. Wherever this file's prose says `.fredrin/memory/…`, read
`docs/…` instead. There is exactly one ADR home — never create
`.fredrin/memory/adr/`.
...
concept doc or a note — if `context-classify` wants one, the term goes in
`CONTEXT.md` instead.

**`AGENTS.md` and `CLAUDE.md` are both symlinks to `README.md`.** Edit
`README.md`; never replace either symlink with a regular file.

### Issue tracking

Issues and tickets for this repo live in **Fredrin**, not GitHub Issues. Do not
use `gh issue` — see `docs/agents/issue-tracker.md`.
