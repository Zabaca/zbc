...
**And "before it" means at the moment the push is published, not the moment it was judged.** `pre-receive` answers before the pack uploads, and for a real repository that upload is seconds — long enough for the name to be claimed underneath it. So the same verdict is asked a second time inside the compare-and-swap that publishes, against the Index that CAS is committing onto, reading the claim as it stands before this push applies its own. That is what keeps the founding push through its own re-check, and it is a sharpening of the rule above rather than a change to it: the earlier asking is where a refusal is free, the later one is where the answer is true. A push spanning several transactions is not judged by the list its own earlier transaction installed either — the Claim it wrote, timestamp and all, is how the later ones recognise it — or a founding push that lists somebody else's key would refuse its own branch. It is the only verdict asked twice — append-only and the size caps judge what was _pushed_, which cannot change underneath the push, while ownership judges the _repository_, which can.

The cost is a refusal reached after the upload, which is exactly what the placement above exists to avoid, and it cannot be avoided here: the race is precisely that the earlier answer went stale. It leaves an Orphan, which `findOrphans` already reclaims, and it says so — the refusal that arrives late explains why it is late and does not repeat the earlier one's "nothing was uploaded". Only `pre-receive`'s output rides the sideband to the client; git kills the connection on a `prepared` hook that exits non-zero, so the late refusal is written for the host's log.
...
- **A claimed repository stays world-readable**, and so do its list and its Provenance. Reads are untouched here. _Revised by [ADR-0013](./0013-a-name-can-refuse-a-stranger-reading.md)_: a claimed name may additionally carry a Reader List, and this ADR turned out to be the prerequisite for that rather than unrelated to it.
