...
Each duplication was annotated as deliberate, and all of them rested on one stated premise — from `worker/landing.ts`: _"the two halves compile against conflicting ambient types … so they share a vocabulary without sharing a bundle."_ The premise had been overtaken by the tree. What is actually true is narrower: `src/` and `worker/` cannot import **each other**, because `@types/bun` and `@cloudflare/workers-types` are contradictory ambient declarations. A module that imports neither is valid under both, and the app had five such modules in production use. The fix follows directly: give that category a home.

**The object-store key namespace had no owner.** `repos/<repo_id>/…` is the one thing every module agrees on and the one thing a bucket cannot be migrated away from, and it was authored in six places and read back by hand in two more. Two consequences: `listRepoIds` lived in `src/usage.ts` — the read-only reporting command — so `src/expire.ts`, whose job is _deleting_ repositories, imported the usage reporter to enumerate them; and `src/gc.ts` reconstructed a ULID out of a WAL key with a `split`, a `pop`, a `replace`, an `indexOf` and a `slice`, reversing a format `walKey` produced six modules away without saying so.
...
- **`src/keys.ts` owns the layout.** Every prefix, every key builder, the `.pack` → `.idx` sibling rule, the parse back out of a WAL key, and `listRepoIds`. `src/expire.ts` no longer imports `src/usage.ts`; `src/gc.ts` asks for a key's upload time instead of dismantling the string.
  ...
  _Amended 2026-09-19._ The layout moved to **`shared/keys.ts`**, with `shared/ulid.ts`, `shared/store.ts` (the `ObjectStore` interface, `MemoryStore`, `S3Store`) and `shared/wal-index.ts` (the Index's types and its reader) beside it. The reason is this ADR's own rule rather than an exception to it: the edge answers questions off `index.json` too, and a second store client or a second idea of what a key means would be exactly the drift the kernel exists to prevent. What could not move stayed in `src/` and is the container's disk — `FileStore`, `sha256` (`Bun.CryptoHasher`) and the compare-and-swap writers.
- **The app template stays dependency-free.** walgit is scaffolded verbatim into consumer repos ([ADR-0005](./0005-subtree-distribution-zbc-core.md)), so `shared/` adds no package and no build step — it is three tsconfig `include` entries and a directory.
  ...
  _Amended 2026-09-19._ No longer true of the template as a whole: `shared/mcp.ts` (the MCP endpoint) imports `@modelcontextprotocol/sdk` and `zod`, and `worker/mcp.ts` imports `@hono/mcp` and `hono`. The rule this ADR actually binds is untouched and is the one that matters — `shared/` imports no **runtime**, and the two tsconfigs still fail the build if it does; a library is not a runtime. What this consequence claimed beyond that, that the kernel would never cost a package, was a property of the modules that existed in 2026-08-29 rather than a rule anything enforced.
...
- **`src/usage.ts` mixes collection with presentation** — it folds indexes into a report _and_ renders that report as terminal text. The seam is obvious, but splitting it is a different kind of change and would be buried in a diff of moved text.
