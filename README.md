...
- **`zbc apply <env> --json <path>`** writes `{ env, instances: [{ name, module,
  ...
  (see below), so a minted credential never reaches the file; anything a module
  did *not* declare is written verbatim, so still treat the file as sensitive
  and delete it after reading, as the workflow does.
